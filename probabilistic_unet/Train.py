        ...
        for epoch in epobar:
            ...
            with tqdm(train_loader, unit="batch", leave=False) as batbar:
                for i, batch in enumerate(batbar):
                    ...
                    seg, priorDists, posteriorDists, friPred = model(
                        batchImg, batchLabel, FriLabel
                    )
                    (
                        loss,
                        kl_mean,
                        ...
                        batchLabel, seg, priorDists, posteriorDists, FriLabel, friPred
                    )
                    ...
                    if configs.GECO.enable:
                        tr_loss["lambda fri coeficient"].append(
                    ...
                    tr_loss["Regression training loss"].append(
                        torch.mean(regLoss.detach().cpu()).item()
                    )
