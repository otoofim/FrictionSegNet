...
def multiclass_iou(output, target, classes):
    ...
    if isinstance(classes, dict):
        ...
        class_items = enumerate(classes)

    for cls, className in class_items:
...
def SingleImageConfusionMatrix(Predictions, Lables, classes):
    ...
    for j, _ in enumerate(classes):
        ...
        for i, _ in enumerate(classes):
            ...
            is_pixel_predicted_as_i_and_is_in_class_j = (
                is_pixel_in_class_j * is_pixel_predicted_as_i
            )
            total_pixels_predicted_as_i_in_class_j = (
                is_pixel_predicted_as_i_and_is_in_class_j.sum()
            )
...
def BatchImageConfusionMatrix(predictions, labels, classes):
    ...
    smooth = 0.0
    num_classes = len(classes)
    pixels_in_class_j_totals = torch.zeros([num_classes, 1])
    CM_unnormalised_totals = torch.zeros([num_classes, num_classes])

    confusionTest = [
        SingleImageConfusionMatrix(pred, sample, classes)
    ...
    CM = torch.tensor(
        [
            100
            * (
                (CM_unnormalised_totals[i][j] + smooth)
                / (pixels_in_class_j_totals[j] + smooth)
            )
            for i, _ in enumerate(classes)
            for j, _ in enumerate(classes)
        ]
